...
/* ── Matches the homepage CollectionImageGrid pattern exactly ── */
function EditorialGrid({ hero, mid, sm, flip }) {
  const { isMobile, isTablet } = useIsMobile();

  if (isMobile) {
    return (
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", width: "100%" }}>
        <div style={{ position: "relative", paddingBottom: "130%", borderRadius: "8px", overflow: "hidden" }}>
          <img src={hero} alt="" draggable={false} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          <div style={{ position: "relative", paddingBottom: "60%", borderRadius: "8px", overflow: "hidden" }}>
            <img src={mid} alt="" draggable={false} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />
          </div>
          <div style={{ position: "relative", paddingBottom: "60%", borderRadius: "8px", overflow: "hidden" }}>
            <img src={sm} alt="" draggable={false} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{
      ...
      {/* Hero — tall, spans both rows on the wider column */}
      <div style={{
        gridColumn: flip ? "2" : "1",
        gridRow: "1 / 3",
        overflow: "hidden",
        ...
        <img src={hero} alt="" draggable={false}
          style={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform 700ms cubic-bezier(0.25,1,0.5,1)" }}
          onMouseEnter={e => e.currentTarget.style.transform = "scale(1.04)"}
          onMouseLeave={e => e.currentTarget.style.transform = "scale(1)"}
        />
      </div>
      {/* Mid */}
      <div style={{ gridColumn: flip ? "1" : "2", gridRow: "1", overflow: "hidden", borderRadius: "8px" }}>
        <img src={mid} alt="" draggable={false}
          style={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform 700ms cubic-bezier(0.25,1,0.5,1)" }}
          onMouseEnter={e => e.currentTarget.style.transform = "scale(1.04)"}
          onMouseLeave={e => e.currentTarget.style.transform = "scale(1)"}
        />
      ...
      {/* Small */}
      <div style={{ gridColumn: flip ? "1" : "2", gridRow: "2", overflow: "hidden", borderRadius: "8px" }}>
        <img src={sm} alt="" draggable={false}
          style={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform 700ms cubic-bezier(0.25,1,0.5,1)" }}
          onMouseEnter={e => e.currentTarget.style.transform = "scale(1.04)"}
          onMouseLeave={e => e.currentTarget.style.transform = "scale(1)"}
        />
...
export default function CollectionDetailPage() {
  ...
  const col = COLLECTION_DATA[id];

  useSEO({
    title: col ? `${col.name} — pictureprefect` : "Collection — pictureprefect",
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F5F1EB", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <section style={{ maxWidth: "1200px", margin: "0 auto", padding: isMobile ? "2.5rem 1.25rem 3rem" : isTablet ? "2.5rem 2rem 3.5rem" : "3rem 2rem 4rem", display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? "2rem" : "4rem", alignItems: "center" }}>
        ...
        <motion.div variants={fadeUp} custom={0.15} initial="hidden" animate="visible">
          ...
          <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap", marginBottom: "1.25rem" }}>
            ...
            <button
              style={{ padding: "1rem 2rem", background: "transparent", color: "#2C2825", border: "1px solid rgba(44,40,37,0.25)", cursor: "pointer", fontSize: "0.72rem", letterSpacing: "0.18em", textTransform: "uppercase", fontFamily: "'DM Sans', sans-serif", fontWeight: 500, borderRadius: "2px", transition: "all 250ms" }}
              onMouseEnter={e => { e.currentTarget.style.background = "#2C2825"; e.currentTarget.style.color = "#F5EDE0"; }}
      ...
      {/* ── TRANSFORMATIONS ───────────────────────────────── */}
      <section style={{ background: "#EDEAE4", padding: "5rem 0" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          ...
          <div className="ba-grid">
            {col.pairs.map((pair, i) => (
              <SliderPair key={i} img={pair.img} mode={revealMode} />
            ))}
          </div>
      ...
      <section style={{ maxWidth: "1200px", margin: "0 auto", padding: "5rem 2rem" }}>
        ...
        <motion.div
          ...
          style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4, 1fr)", gap: "1px", background: "rgba(44,40,37,0.08)", borderRadius: "12px", overflow: "hidden" }}
        >
          {[
            { icon: "◈", label: "Presets", value: `${col.count}`, sub: "unique looks" },
            { icon: "◇", label: "Style", value: col.style.split(" · ")[0], sub: col.style.split(" · ").slice(1).join(" · ") },
            { icon: "◉", label: "Format", value: "XMP + DNG", sub: "industry standard" },
            { icon: "○", label: "Compatible", value: "Mobile + Desktop", sub: "Lightroom & Camera Raw" },
          ].map(d => (
            <div key={d.label} style={{ background: "#F5F1EB", padding: "2rem 1.5rem", textAlign: "center" }}>
              <div style={{ color: "#B08D5B", fontSize: "1.2rem", marginBottom: "0.75rem", lineHeight: 1 }}>{d.icon}</div>
              <div style={{ color: "#9A9088", fontSize: "0.55rem", letterSpacing: "0.22em", textTransform: "uppercase", fontFamily: "'DM Sans', sans-serif", marginBottom: "0.4rem" }}>{d.label}</div>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.1rem", color: "#2C2825", fontWeight: 500, marginBottom: "0.2rem" }}>{d.value}</div>
              <div style={{ color: "#9A9088", fontSize: "0.7rem", fontWeight: 300 }}>{d.sub}</div>
            </div>
          ))}
        </motion.div>
        ...
        <motion.div
          ...
          style={{ marginTop: "2.5rem", display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: "0.75rem 3rem" }}
        >
          {[
            `${col.count} Lightroom Presets (.XMP)`,
            "Mobile DNG Profiles",
            "Camera Raw Compatible",
            "Wedding Day Workflow PDF",
            "One-click apply — no manual adjustments",
            "Lifetime access · Free updates",
          ].map(item => (
            <div key={item} style={{ display: "flex", alignItems: "flex-start", gap: "0.75rem", padding: "0.6rem 0", borderBottom: "1px solid rgba(44,40,37,0.07)" }}>
              <div style={{ width: "5px", height: "5px", borderRadius: "50%", background: "#B08D5B", flexShrink: 0, marginTop: "0.35rem" }} />
              <span style={{ color: "#5A544D", fontSize: "0.85rem", fontWeight: 300, lineHeight: 1.5 }}>{item}</span>
            </div>
          ))}
        </motion.div>
      </section>
      ...
      <section style={{ background: "#EDEAE4", padding: "5rem 0" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 2rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            <motion.div
              variants={fadeUp} custom={0} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }}
            >
              <EditorialGrid
                hero={col.gallery[0]}
                mid={col.gallery[1]}
            ...
            <motion.div
              variants={fadeUp} custom={0.1} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }}
            >
              <EditorialGrid
                hero={col.gallery[3]}
                mid={col.gallery[4]}
