...
const ASPECT = {
  tall:  "75% 0 0 / 133%",   // 2:3 portrait — paddingBottom trick → use below
  wide:  "56.25%",            // 16:9
  reel:  "177.78%",           // 9:16 vertical
  sq:    "100%",              // square
};

const paddingByAspect = {
  tall: "133%",
  wide: "65%",
  reel: "175%",
  sq:   "100%",
...
function VideoTile({ item, onClick }) {
  ...
  return (
    <div
      ...
      onClick={onClick}
      style={{ position: "relative", width: "100%", paddingBottom: paddingByAspect[item.aspect], cursor: "pointer", overflow: "hidden", background: "#1a1614" }}
    >
      <video
        ref={ref}
        src={item.src}
      ...
      <div style={{
        ...
        {hovered && (
          <div style={{ opacity: 1, transform: "scale(1)", transition: "all 300ms" }}>
            <PlayIcon />
          </div>
        )}
      </div>
...
function ImageTile({ item, onClick }) {
  ...
  return (
    <div
      ...
      onMouseLeave={() => setHovered(false)}
      onClick={onClick}
      style={{ position: "relative", width: "100%", paddingBottom: paddingByAspect[item.aspect], cursor: "pointer", overflow: "hidden", background: "#2C2825" }}
    >
      <img
        src={item.src}
        alt={item.alt}
...
export default function GalleryPage() {
  useSEO({
    title: "Gallery — pictureprefect",
    description: "Browse real wedding images edited with pictureprefect Lightroom presets. See the transformation across ceremonies, portraits, receptions, and golden hour shots.",
    path: "/gallery",
  });
