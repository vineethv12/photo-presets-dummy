      ...
      <motion.div initial={{ scale: 0.94, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.94, opacity: 0 }}
        ...
        <div style={{ borderRadius: "10px", overflow: "hidden", boxShadow: "0 32px 80px rgba(0,0,0,0.55)", display: "flex", flexDirection: "column" }}>
          <div style={{ aspectRatio: "16/9", position: "relative", flexShrink: 0 }}>
            <iframe key={activeId} src={`https://www.youtube.com/embed/${activeId}?autoplay=1&rel=0`} title="Video"
...
export default function ProcessPage() {
  useSEO({
    title: "Our Creative Process — pictureprefect",
    description: "Discover how pictureprefect presets are built — from raw capture and colour correction to skin tone refinement, cinematic grading, and final delivery.",
    path: "/process",
  });
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", color: C.dark, fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <section style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: `linear-gradient(160deg, ${C.bg} 0%, ${C.bg2} 60%, ${C.bg3} 100%)`, paddingTop: "calc(var(--nav-h) + 2.5rem)", position: "relative", overflow: "hidden" }}>
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.1, ease: [0.22, 1, 0.36, 1] }}
          ...
          <p style={{ color: C.mid, fontSize: "clamp(0.85rem, 1.3vw, 1rem)", fontWeight: 300, lineHeight: 1.8, maxWidth: "440px", margin: "0 auto 2.5rem" }}>
            From raw footage to cinematic perfection — the discipline, decisions, and details behind every pictureprefect preset.
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 1, ease: [0.22, 1, 0.36, 1] }}
          ...
          <motion.div
            ...
            <img src={`https://img.youtube.com/vi/${FEATURED_VIDEO_ID}/maxresdefault.jpg`} alt="Featured" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
            {/* gradient overlay */}
            ...
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "1.5rem 1.75rem", display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: "1rem" }}>
              <div>
                ...
                <p style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(1rem, 1.8vw, 1.25rem)", color: "#F5F1EB", lineHeight: 1.3 }}>Inside pictureprefect — Full Edit Walkthrough</p>
      ...
      <AnimatePresence>
        {featuredOpen && <VideoModal videoId={FEATURED_VIDEO_ID} onClose={() => setFeaturedOpen(false)} />}
